# -*- coding: utf-8 -*-
# ising_model.py
# (Modified version with increased temperature sampling points)
...
def run_simulation(L: int, beta: float, num_equilibration_sweeps: int, num_measurement_sweeps: int,
    ...
    # print(f"equilibration_steps: {equilibration_steps}, type: {type(equilibration_steps)}") # Debug print removed

    measurement_steps = num_measurement_sweeps * N * N
    # print(f"measurement_steps: {measurement_steps}, type: {type(measurement_steps)}") # Debug print removed
    ...
    if not magnetization_measurements:
        ...
        if len(measurements_array) >= bin_size:
            ...
            print(f" Warning: Only {len(measurements_array)} measurements, less than bin_size={bin_size}. Not using binning.")
        ...
        # Avoid division by zero or issues if all M are the same
        variance_M = mean_M_sq - np.mean(measurements_array)**2
        if variance_M < 1e-12: # Handle potential floating point issues near zero variance
            susceptibility = 0.0
        else:
            susceptibility = beta * (N**2) * variance_M
...
def create_ising_animation(L: int, beta: float, num_frames: int, interval_sweeps: int,
    ...
    equilibration_steps = equilibrate_sweeps * N * N # Renamed variable to avoid conflict
    eq_accepted = metropolis_sweep(lattice, N, beta, equilibration_steps)
    eq_acceptance_rate = eq_accepted / equilibration_steps if equilibration_steps > 0 else 0
    print(f"Equilibration finished. Acceptance rate: {eq_acceptance_rate:.4f}")
    ...
    # plt.close()  # Prevents the empty figure from being displayed if running non-interactively, BUT keep it open for animation display in interactive modes
    ...
    # Use interval=50 for faster update, blit=True for efficiency
    ani = animation.FuncAnimation(fig, update, frames=num_frames, interval=50, blit=True)
    
    # Save the animation using imageio (pillow is often slower for GIFs)
    filename = f"ising_animation_L{L}_beta{beta:.3f}.gif"
    print(f"Creating animation with {num_frames} frames...")
    try:
        # Using imageio writer directly might give more control if needed
        # writer = imageio.get_writer(filename, mode='I', duration=0.2) # duration per frame
        # for i in range(num_frames):
        #     update(i) # update the state
        ...
        # Using FuncAnimation.save with pillow (might need testing for backend)
        # ani.save(filename, writer='pillow', fps=10) # fps = frames per second

        # Using FuncAnimation.save with imageio (Recommended)
        ani.save(filename, writer='imageio', fps=10) # fps = frames per second

        print(f"Animation saved to {filename}")
    except Exception as e:
        print(f"Error saving animation: {e}")
        print("Try installing 'imageio[ffmpeg]' or 'imageio[pyav]' if issues persist.")
...
def run_simulation_with_snapshots(L, beta, num_equilibration_sweeps, num_measurement_sweeps,
    ...
    # Use optimized sweep function
    metropolis_sweep(lattice, N, beta, equilibration_steps)
    ...
    # Calculate snapshot intervals (ensure at least 1 sweep between snapshots)
    snapshot_interval_sweeps = max(1, num_measurement_sweeps // num_snapshots)
    snapshot_interval_steps = snapshot_interval_sweeps * N * N
    next_snapshot_step = snapshot_interval_steps # Step number to take the next snapshot
    snapshot_count = 0

    # Use optimized sweep function for intervals
    total_measured_steps = 0
    while total_measured_steps < measurement_steps:
        steps_to_run = min(snapshot_interval_steps, measurement_steps - total_measured_steps)
        if steps_to_run <= 0:
            break

        metropolis_sweep(lattice, N, beta, steps_to_run)
        total_measured_steps += steps_to_run

        # Take measurement (e.g., after each interval)
        current_M = calculate_magnetization(lattice, N)
        magnetization_measurements.append(current_M)
        ...
        if snapshot_count < num_snapshots:
             snapshot_count += 1
             print(f"  Taking snapshot {snapshot_count} at step {total_measured_steps}")
             npy_file, png_file = save_lattice_snapshot(lattice, L, beta, snapshot_count)
    ...
    mean_M = np.mean(np.abs(measurements_array))
    mean_M_sq = np.mean(measurements_array**2)
    susceptibility = beta * (N**2) * (mean_M_sq - np.mean(measurements_array)**2) # Use population variance if M=0
...
def visualize_snapshots_grid(snapshot_files, title, output_filename):
    ...
    if n_snapshots == 0:
        print("No snapshot files provided.")
        return
    ...
    if rows == 1 and cols == 1:
        axes = np.array([[axes]]) # Ensure axes is always 2D for consistent indexing
         ...
         axes = axes.reshape(rows, cols) # Ensure correct shape for flatten later if needed
    ...
    for i, (npy_file, _) in enumerate(snapshot_files):
        if i < len(axes_flat):
            try:
                ...
                id_match = re.search(r'id(\d+)', npy_file)
                id_str = id_match.group(1) if id_match else f"{i+1}"

                # Plot on the grid
                axes_flat[i].imshow(lattice, cmap='binary', vmin=-1, vmax=1, interpolation='nearest')
                axes_flat[i].set_title(f"Snapshot {id_str} (β={beta_str})")
                axes_flat[i].set_xticks([])
                axes_flat[i].set_yticks([])
            except FileNotFoundError:
                 print(f"Warning: Snapshot file not found: {npy_file}")
                 axes_flat[i].set_title(f"File not found:\n{os.path.basename(npy_file)}")
                 axes_flat[i].axis('off')
            except Exception as e:
                 print(f"Warning: Error loading or plotting {npy_file}: {e}")
                 axes_flat[i].set_title(f"Error loading:\n{os.path.basename(npy_file)}")
                 axes_flat[i].axis('off')
...
if __name__ == '__main__':
    ...
    # INCREASED NUMBER OF POINTS for higher resolution
    T_low = np.linspace(0.5, 2.1, 30, endpoint=False) # Increased from 15 to 30 points
    T_crit = np.linspace(2.1, 2.4, 50) # Increased from 25 to 50 points
    T_high = np.linspace(2.4, 5.0, 30, endpoint=True)[1:] # Increased from 15 to 30 points (approx 29 used)
    temp_values = np.unique(np.concatenate((T_low, T_crit, T_high))) # Combine and remove duplicates
    num_temp_points = len(temp_values) # Get the actual number of points (~109 points)
    ...
    if MATPLOTLIB_AVAILABLE and results_T:
        ...
        axs[0].plot(results_T, results_E, 'o', markersize=4, linestyle='-', color='indianred', label='Simulation Data')
        ...
        axs[1].plot(results_T, results_M, 'o', markersize=4, linestyle='-', color='royalblue', label='Simulation Data $\\langle |M| \\rangle$')
        ...
        axs[2].plot(results_T, results_Chi, 'o', markersize=4, linestyle='-', color='mediumseagreen', label='Simulation Data $\\chi$')
        ...
        # Update plot filename to reflect the enhanced methodology and potentially more points
        plot_filename = f'ising_E_M_Chi_vs_T_L{param_L}_Eq{param_eq_sweeps}_Me{param_meas_sweeps}_more_points.png'
    ...
    if MATPLOTLIB_AVAILABLE and IMAGEIO_AVAILABLE:
        ...
        # Define specific beta/T values for animation
        low_T = 1.67
        crit_T = Tc_exact
        high_T = 5.0
        ...
        low_beta = 1.0 / low_T
        print(f"\nGenerating animation for low temperature (T={low_T:.4f}, beta={low_beta:.4f})...")
        ...
        crit_beta = 1.0 / crit_T
        print(f"\nGenerating animation for critical temperature (T={crit_T:.4f}, beta={crit_beta:.4f})...")
        create_ising_animation(
            L=param_L,
            beta=crit_beta,
            num_frames=animation_frames,
        ...
        high_beta = 1.0 / high_T
        print(f"\nGenerating animation for high temperature (T={high_T}, beta={high_beta:.4f})...")
        create_ising_animation(
            L=param_L,
            beta=high_beta,
            num_frames=animation_frames,
